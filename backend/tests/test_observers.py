...
def test_logging_observer_routes_warning_events_to_warning_level(caplog):
    ...
    The assertion is filtered to the `lidenbrock.core` logger and
    ...
    the lidenbrock.core logger.
    """
    observer = LoggingObserver()

    with caplog.at_level(logging.DEBUG, logger="lidenbrock.core"):
    ...
    warning_records = [
        r for r in caplog.records if r.levelname == "WARNING" and r.name == "lidenbrock.core"
    ]
    ...
    for event_type in ("warning", "chunk_error", "hyphen_partner_missing"):
        assert event_type in messages, (
            ...
            f"record on lidenbrock.core; got messages: {messages!r}"
        )
...
def test_logging_observer_routes_lifecycle_events_to_debug_level(caplog):
    ...
    The DEBUG-count check is filtered to the `lidenbrock.core`
    ...
    assert warning_records == [], (
        f"lifecycle events leaked to WARNING on lidenbrock.core: "
        f"{[(r.levelname, r.message) for r in warning_records]}"
    )
    debug_messages = " ".join(
        r.message for r in caplog.records if r.levelname == "DEBUG" and r.name == "lidenbrock.core"
    )
    for event_type in lifecycle_events:
        assert event_type in debug_messages, (
            ...
            f"log record on lidenbrock.core; got messages: {debug_messages!r}"
        )
